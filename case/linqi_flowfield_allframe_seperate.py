...
np.save(f'{file_name}_gt_velocity_magnitude.npy', gt_cube)
np.save(f'{file_name}_pred_velocity_magnitude.npy', pred_cube)
print("Saved velocity magnitude fields")

# pred_cube = np.load(f'{file_name}_pred_velocity_magnitude.npy')
# gt_cube = np.load(f'{file_name}_gt_velocity_magnitude.npy')

global_min = np.percentile(
    np.concatenate([gt_cube.flatten(), pred_cube.flatten()]), 0.111)
global_max = np.percentile(
    np.concatenate([gt_cube.flatten(), pred_cube.flatten()]), 99.9999)
...
if not save_fig:
    ...
    p.camera.azimuth = 0
    p.open_gif(f"{file_name}_flowfield_all_sep_gt.gif", fps=15)
    p.camera.zoom(0.9)
...
#     show_zlabels=False,
# )
p2.add_text("Prediction", font_size=20)
# Calculate global min/max for consistent color scaling
# Calculate 5th and 95th percentiles across both arrays
...
p2.add_mesh(rock_mesh, color="gray", opacity=0.1)
...
if not save_fig:
    ...
    p2.camera.azimuth = -30
    p2.camera.elevation = 20

    p2.camera.azimuth = 0
    p2.open_gif(f"{file_name}_flowfield_all_sep_pd.gif", fps=15)
    p2.camera.zoom(0.9)
    for i in range(num_frames):
        p2.camera.azimuth = p2.camera.azimuth - 1
        p2.write_frame()
    p2.close()
